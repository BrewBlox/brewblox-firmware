...
lcov -q --remove coverage/total.info '/usr/*' 'cnl/*' -o coverage/filtered.info

echo "generating html"
mkdir -p coverage/html
genhtml -q --prefix /firmware/lib coverage/filtered.info --ignore-errors source --output-directory=coverage/html
popd